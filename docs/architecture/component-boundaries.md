...
Jinja2 기반 데모 웹 UI는 FastAPI의 표현 계층이며 다음 기능만 제공한다.

* 자연어 질문 입력과 제한된 결과·XAI 표시
* Business·Physical Metadata 읽기 전용 조회
* Backend가 정의한 승인 Dataset의 AdventureWorks Sample 데이터 조회

웹 UI는 대상 DB 드라이버를 사용하지 않고 임의 SQL 입력을 제공하지 않는다. Metadata와 Sample 데이터 조회도 FastAPI의 정책과 승인된 MCP Tool 경계를 따른다.
...
* 대상 DB Schema Inspection과 Physical Metadata 원본 반환
