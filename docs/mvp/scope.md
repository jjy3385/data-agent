...
> 이 문서는 4주 MVP에서 구현할 업무 범위, 대표 질문, 테스트 역할과 의도적으로 제외한 기능을 정의한다. 새로운 요구사항이 들어왔을 때 지금 구현할지 Post-MVP로 미룰지 판단하는 기준이다.
...
MVP에서 허용하는 실행 모드는 `private`뿐이다.

```dotenv
LLM_DEPLOYMENT_MODE=private
LLM_BASE_URL=<approved-private-llm-endpoint>
LLM_MODEL=<private-model-name>
```

`LLM_DEPLOYMENT_MODE=public`은 Post-MVP 예약값이다. 애플리케이션은 설정값만으로 Endpoint가 Private이라고 신뢰하지 않고 승인 Endpoint Allowlist, 네트워크 경로, TLS와 인증을 함께 검증한다. Private LLM에도 Raw Result 전체를 반복 전송하지 않고 최소 Result Projection만 제공한다.
...
* Public LLM용 Result Handle과 데이터 격리
...
Public LLM Result Handle의 상세 결정은 [ADR 0006](../adr/0006-result-handle.md)을 따른다.
